...
parser.add_argument('-n', '--n_runs', action='store', type=int, default=4)
parser.add_argument('-s', '--starting_run', action='store', type=int, default=0)
...
params = [
    '--env_name capture_target --gridx 3 --gridy 3 --n_quant 16 ' + \
    '--implicit 1 --likely 1 --distort_type wang --distort_param 0.0  --buffer_size 4000 --run_id {}{}'.format(
        '' if args.task_id < 0 else '{}-'.format(args.task_id), run)
    for run in range(args.starting_run, args.starting_run + args.n_runs)
